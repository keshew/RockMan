...
extension RockManGameSpriteKit {
    ...
    func swipeWith(item: FlyingItem, button: SKSpriteNode, touches: Set<UITouch>) {
        let arrowTopTexture = String(describing: SKTexture(imageNamed: RockImageName.arrowTop.rawValue))
        let arrowButtomTexture = String(describing: SKTexture(imageNamed: RockImageName.arrowBottom.rawValue))
        let arrowLeftTexture = String(describing: SKTexture(imageNamed: RockImageName.arrowLeft.rawValue))
        let arrowRightTexture = String(describing: SKTexture(imageNamed: RockImageName.arrowRight.rawValue))
            ...
            if abs(swipeVector.dx) > abs(swipeVector.dy) {
                if swipeVector.dx > 0 {
                    changeImage(item: item,
                                ...
                                button: button,
                                idealImage: RockImageName.arrowRightGreen.rawValue,
                                avverageImage: RockImageName.arrowRightOrange.rawValue,
                                badImage: RockImageName.arrowRightRed.rawValue)
                } else {
                    changeImage(item: item,
                                tapTexture: arrowLeftTexture,
                                button: button,
                                idealImage: RockImageName.arrowLeftGreen.rawValue,
                                avverageImage: RockImageName.arrowLeftOrange.rawValue,
                                badImage: RockImageName.arrowLeftRed.rawValue)
                }
            } else {
                if swipeVector.dy > 0 {
                    changeImage(item: item,
                                ...
                                button: button,
                                idealImage: RockImageName.arrowTopGreen.rawValue,
                                avverageImage: RockImageName.arrowTopOrange.rawValue,
                                badImage: RockImageName.arrowTopRed.rawValue)
                } else {
                    changeImage(item: item,
                                tapTexture: arrowButtomTexture,
                                button: button,
                                idealImage: RockImageName.arrowBottomGreen.rawValue,
                                avverageImage: RockImageName.arrowBottomOrange.rawValue,
                                badImage: RockImageName.arrowBottomRed.rawValue)
                }
            }
        }
    ...
    func tapWith(item: FlyingItem, button: SKSpriteNode) {
        let tapTexture = String(describing: SKTexture(imageNamed: RockImageName.tapButton.rawValue))
        changeImage(item: item,
                    tapTexture: tapTexture,
                    button: button,
                    idealImage: RockImageName.greenTapButton.rawValue,
                    avverageImage: RockImageName.orangeTapButton.rawValue,
                    badImage: RockImageName.redTapButton.rawValue)
    }
    ...
    func checkForLosesNote(item: FlyingItem) {
        ...
        let blueNote = String(describing: SKTexture(imageNamed: RockImageName.blueNote.rawValue))
        let redNote = String(describing: SKTexture(imageNamed: RockImageName.redNote.rawValue))
        let pinkNote = String(describing: SKTexture(imageNamed: RockImageName.pinkNote.rawValue))
        let greenNote = String(describing: SKTexture(imageNamed: RockImageName.greenNote.rawValue))
    ...
    func checkForBrokeItem(item: FlyingItem) {
        let eggTexture = String(describing: SKTexture(imageNamed: RockImageName.egg.rawValue))
        let egg1Texture = String(describing: SKTexture(imageNamed: RockImageName.egg1.rawValue))
        let tomatoTexture = String(describing: SKTexture(imageNamed: RockImageName.tomato.rawValue))
        let itemTexture = String(describing: item.texture!)
        
        if itemTexture == eggTexture {
            game?.countOfLives -= 0.15
            gameScore()
            let brokenEgg = SKSpriteNode(imageNamed: RockImageName.eggBroken.rawValue)
        ...
        if itemTexture == tomatoTexture {
            game?.countOfLives -= 0.15
            gameScore()
            let tomatoBroken = SKSpriteNode(imageNamed: RockImageName.tomatoBroken.rawValue)
    ...
    func getRandomFlyingItem() -> [String] {
        let allItems: [String] = [RockImageName.money.rawValue,
                                          RockImageName.blueNote.rawValue,
                                          RockImageName.redNote.rawValue,
                                          RockImageName.pinkNote.rawValue,
                                          RockImageName.egg.rawValue,
                                          RockImageName.egg1.rawValue,
                                          RockImageName.tomato.rawValue]
        return allItems
    }
    
    func getRandomBack() -> [String] {
        let allItems: [String] = [RockImageName.gameBackground.rawValue,
                                  RockImageName.gameBackground2.rawValue,
                                  RockImageName.gameBackground3.rawValue,
                                  RockImageName.gameBackground4.rawValue,
                                  RockImageName.gameBackground5.rawValue]
        return allItems
    }
    
    func getRandomMethodToCatch() -> [String] {
        let allItems: [String] = [RockImageName.arrowTop.rawValue,
                                  RockImageName.arrowLeft.rawValue,
                                  RockImageName.arrowRight.rawValue,
                                  RockImageName.arrowBottom.rawValue,
                                  RockImageName.tapButton.rawValue]
        return allItems
    }
    ...
    func setupView() {
        ...
        let menuBackground = SKSpriteNode(imageNamed: RockImageName.smallButtonBackground.rawValue)
        ...
        let menu = SKSpriteNode(imageNamed: RockImageName.menu.rawValue)
        menu.size = CGSize(width: size.width / 10, height: size.height / 25)
        menu.zPosition = 10
        menu.position = (CGPoint(x: size.width / 7.1, y: size.height / 1.15))
        addChild(menu)
        
        let moneyBackground = SKSpriteNode(imageNamed: RockImageName.buttonBackground.rawValue)
        ...
        let money = SKSpriteNode(imageNamed: RockImageName.money.rawValue)
        ...
        rectangle = SKSpriteNode(imageNamed: RockImageName.rectangle.rawValue)
        ...
        let rockMan = SKSpriteNode(imageNamed: RockImageName.rockMan.rawValue)
    ...
    func gameScore() {
        if game!.countOfLives < 3.0 {
            heart3.texture = SKTexture(imageNamed: RockImageName.grayHeart.rawValue)
        }
        
        if game!.countOfLives < 2.0 {
            heart2.texture = SKTexture(imageNamed: RockImageName.grayHeart.rawValue)
        }
    }
}
